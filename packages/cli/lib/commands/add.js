...
module.exports = async function () {
  ...
  const modulePath = path.join(paths.appModules, moduleName);
  ...
  try {
    ...
    console.error("Git is not installed; can't add a MagicMirror module.");
    return 1;
  }
  ...
  console.info(
    ...
    }. Check ${chalk.underline(
      humanUrl
    )} to see if there are any other installation instructions.`
  );
  console.info(
    `Open ${chalk.underline(paths.appConfig)} to add it to your config file.`
  );
};
