...
describe('useMemoArray', () => {
  test("memoizes handler", () => {
    ...
    const { result, rerender } = renderHook((props) => useMemoArray(props.array, handler, []), {
      initialProps: {
        array: [1, 2, 3],
      }
    });
  ...
  test("changed deps invalidate memoization", () => {
    ...
    const { result, rerender } = renderHook((props) => useMemoArray(props.array, handler, props.deps), {
      initialProps: {
        array: [1, 2, 3],
        deps: [inc]
      }
