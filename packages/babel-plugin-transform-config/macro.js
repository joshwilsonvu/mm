...
function transformConfigMacro({ state, babel, references }) {
  ...
  paths.forEach(path => {
    ...
    assert(args.length >= 1 && args.length <= 2, `${path.node.name} should be called with 1-2 arguments`);
  ...
  paths.forEach(path => {
    ...
    if (optionsLiteral) {
      ...
      optionsLiteral.get("properties")
        .forEach(property => {
          assert(!property.node.computed, "Can't use a computed property in macro options");
          property.get("value").assertStringLiteral();

          const keyPath = property.get("key");
          const key = keyPath.isStringLiteral() ? keyPath.node.value : /* identifier */ keyPath.node.name;
